...
def genStruct(name="Structure", **kwargs):
    return type(
        ...
        dict(
            ...
            % (name, ",".join("%s=%s" % (k, getattr(self, k)) for k in kwargs)),
        ),
    )
...
class MouseBlocker:
    ...
    def hookProc(nCode, wParam, lParam):
        ...
        if nCode < 0:
            return windll.user32.CallNextHookEx(
                None, nCode, WPARAM(wParam), LPARAM(lParam)
            )
    ...
    def start_in_thread(self):
        if self.hook is None:
            self.hook = windll.user32.SetWindowsHookExW(
                win32con.WH_MOUSE_LL, MouseBlocker.hookProc, None, 0
            )
